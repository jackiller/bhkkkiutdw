    ...
    CloneYa.prototype = {
        init: function () {
            ...
            $this.$elem.on('click.' + name, $this.config.cloneThis + '>' + $this.config.deleteButton, function (event) {
                ...
                //$this.$elem.triggerAll('clone_delete delete.' + name, [toDelete]); //*9-> comment delete element standard

				//*9-> add for re-generate id when delete form
				//alert(toDelete.attr('id'));
				var cloneCount = toDelete.closest($this.elem).closestChild($this.config.cloneThis).length;
				if (cloneCount > $this.config.minimum) {
					toDelete.css('background-color', '#FFF2F0');
					toDelete.slideToggle('slow', function () {
						toDelete.remove();
						$this._redoIDs();
					});
				} else {
				...
				//*9<- end for re-generate id when delete form
            });
            ...
            $this.$elem.on('delete.' + name, function (event, toDelete) {
                ...
                if (cloneCount > $this.config.minimum) {
					...
					$this._redoIDs(); //*9-> add for re-generate id when delete form
